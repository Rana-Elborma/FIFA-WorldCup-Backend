...
router.get('/summary', auth_1.requireAuth, async (req, res) => {
    ...
    if (metricsRes.error)
        return res.status(500).json({ error: metricsRes.error.message });
    if (predictionsRes.error)
        return res.status(500).json({ error: predictionsRes.error.message });
    if (alertsRes.error)
        return res.status(500).json({ error: alertsRes.error.message });
    if (gatesRes.error)
        return res.status(500).json({ error: gatesRes.error.message });
