/**
 * Stadium + Zone + Gate routes
 * GET  /stadiums              → list all stadiums
 * GET  /stadiums/:id/zones    → zones in a stadium
 * GET  /stadiums/:id/gates    → all gates in a stadium (via zones)
 * POST /stadiums              → create stadium (admin)
 * PATCH /gates/:id            → open/close gate — issues gate_command (operator+)
 */
import { Router, Request, Response } from 'express';
import { supabase, supabaseAdmin } from '../config/supabase';
import { requireAuth } from '../middleware/auth';
import { requireRole } from '../middleware/rbac';

const router = Router();

// ── GET /stadiums ─────────────────────────────────────────────────────────────
router.get('/', requireAuth, async (_req, res: Response) => {
  const { data, error } = await supabase.from('stadium').select('*');
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data);
});

// ── GET /stadiums/:id/zones ───────────────────────────────────────────────────
router.get('/:id/zones', requireAuth, async (req: Request, res: Response) => {
  const { data, error } = await supabase
    .from('zone')
    .select('*')
    .eq('stadium_id', req.params.id);
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data);
});

// ── GET /stadiums/:id/gates ───────────────────────────────────────────────────
router.get('/:id/gates', requireAuth, requireRole('operator'), async (req: Request, res: Response) => {
  const { data, error } = await supabase
    .from('gate')
    .select('*, zone!inner(stadium_id)')
    .eq('zone.stadium_id', req.params.id);
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data);
});

// ── POST /stadiums ────────────────────────────────────────────────────────────
router.post('/', requireAuth, requireRole('admin'), async (req: Request, res: Response) => {
  const { name, city, capacity } = req.body as { name: string; city: string; capacity: number };
  if (!name || !city || !capacity) {
    return res.status(400).json({ error: 'name, city, capacity are required' });
  }

  const { data, error } = await supabaseAdmin
    .from('stadium')
    .insert({ name, city, capacity })
    .select()
    .single();
  if (error) return res.status(500).json({ error: error.message });

  await supabaseAdmin.from('audit_log').insert({
    user_id:    req.user!.id,
    table_name: 'stadium',
    operation:  'INSERT',
    record_id:  data.stadium_id,
    payload:    { name, city, capacity },
  });

  return res.status(201).json(data);
});

// ── PATCH /gates/:id ──────────────────────────────────────────────────────────
// Issues a gate_command; trigger in DB syncs gate.is_open automatically.
router.patch('/gates/:id', requireAuth, requireRole('operator'), async (req: Request, res: Response) => {
  const { command_type, stadium_id } = req.body as {
    command_type: 'open' | 'close' | 'restrict' | 'emergency_close';
    stadium_id: string;
  };

  if (!command_type || !stadium_id) {
    return res.status(400).json({ error: 'command_type and stadium_id are required' });
  }

  const { data, error } = await supabaseAdmin
    .from('gate_command')
    .insert({
      gate_id:      req.params.id,
      stadium_id,
      command_type,
      issued_by:    req.user!.id,
    })
    .select()
    .single();

  if (error) return res.status(500).json({ error: error.message });
  return res.status(201).json(data);
});

export default router;
